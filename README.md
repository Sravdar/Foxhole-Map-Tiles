...
The `with_json` branch carries two generated manifests so apps can update only the tiles that
...
https://raw.githubusercontent.com/Kastow/Foxhole-Map-Tiles/with_json/info.files.json
```

Suggested client flow: fetch `info.json`, compare the root `hash` with the stored one, and only
when it differs fetch `info.files.json` and download the files whose hash changed. Folder hashes
let you skip whole layers or zoom levels before looking at individual files.
